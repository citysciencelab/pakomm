...
{
    // Zeile 46 musste auskommentiert werden: [HelpURL(XRHelpURLConstants.k_XRTintInteractableVisual)]


    public class XRTintInteractableVisualPaKOMM : XRTintInteractableVisualUnity
    {
        ...
        void LinkAllGORenderers()
        {
            List<Renderer> allGORenderers = new();

            Renderer goLevelRenderer = this.gameObject.GetComponent<Renderer>();
            if (goLevelRenderer != null)
            {
                allGORenderers.Add(goLevelRenderer);
            }

            foreach (Renderer objectRenderer in this.gameObject.GetComponentsInChildren<Renderer>())
            {
                allGORenderers.Add(objectRenderer);
            }

            tintRenderers = allGORenderers;
        }
    }
    ...
    public class XRTintInteractableVisualUnity : MonoBehaviour
    {
